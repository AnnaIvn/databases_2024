...
SELECT SUM(pr.supplier_price), h.hardness
FROM price AS pr
JOIN wood_types AS wt ON pr.woodtype_id = wt.id 
JOIN wood_characteristics AS wch ON wch.id = wt.char_id 
JOIN hardness_char AS h ON h.id = wch.hardness_id
WHERE wch.hardness_id = 1
GROUP BY h.hardness
...
-- в різних комбінаціях та варіаціях. Були задіяні шість таблиць, що були заповнені певними 
-- даними, для роботи з SELECT. Були вокористані різні аргументи функції SELECT для фільлтації
-- даних за певними критеріями. 

-- SELECT column1, column2, ...        вибір колонок
-- FROM table_name                     вибір таблиці
-- WHERE condition                     умова перед групуванням
-- GROUP BY column1                    групування за однією за колонок
-- HAVING condition                    умова після групування
-- ORDER BY column1 ASC/DESC           порядок виводу
-- LIMIT n OFFSET m;                   ліміт на к-сть виведених рядків
